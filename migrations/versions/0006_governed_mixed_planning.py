...
def downgrade() -> None:
    ...
    op.execute("DROP FUNCTION app.claim_agent_task(text)")
    op.execute("ALTER TABLE app.agent_tasks DROP CONSTRAINT agent_tasks_operation_check")
    op.execute("ALTER TABLE app.agent_tasks ADD CONSTRAINT agent_tasks_operation_check CHECK (operation = 'generate_planning_run_v1')")
    op.execute("ALTER TABLE app.agent_executions DROP CONSTRAINT agent_executions_adapter_pair_check")
    op.execute("ALTER TABLE app.agent_executions ADD CONSTRAINT agent_executions_adapter_id_check CHECK (adapter_id = 'deterministic_planning')")
    op.execute("ALTER TABLE app.agent_executions ADD CONSTRAINT agent_executions_adapter_version_check CHECK (adapter_version = 'm4a-v1')")
