...
RELEASE_HOW_TO_TOKENS = (
    ...
    "Gate D",
    "Gate E",
    f'git -C "$repo_root" cat-file -t {RELEASE_TAG}',
    f'git -C "$repo_root" rev-parse {RELEASE_TAG}^{{tag}}',
    f'git -C "$repo_root" rev-parse {RELEASE_TAG}^{{commit}}',
    ...
    f"--prefix={RELEASE_ARCHIVE_ROOT}/",
    f'test ! -e "$tmp_dir/extracted/{RELEASE_ARCHIVE_ROOT}/.git"',
    f'cd "$tmp_dir/extracted/{RELEASE_ARCHIVE_ROOT}"',
    'git -C "$repo_root" fetch origin --tags --prune',
    'test "$(git -C "$repo_root" rev-parse origin/main)" = "$expected_commit"',
    f'test "$(git -C "$repo_root" rev-parse {RELEASE_TAG}^{{commit}})" = "$expected_commit"',
    "annotated tag",
    "GitHub Release",
...
def verify_alembic_contract() -> None:
    ...
    for path in sorted(migration_dir.glob("[0-9][0-9][0-9][0-9]_*.py")):
        ...
        if not isinstance(revision, str) or revision in revisions:
            raise SystemExit("Alembic revision graph is invalid")
