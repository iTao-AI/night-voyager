...
- MKE compatibility reviewers: inspect the [M4B design](superpowers/specs/2026-07-13-m4b-mke-readonly-consumer-design.md), [implementation plan](superpowers/plans/2026-07-13-m4b-mke-readonly-consumer.md), and [MKE Evidence boundary ADR](decisions/0005-mke-readonly-evidence-boundary.md). Both the M4B design and implementation plan are approved, but M4B is not implemented; the default evaluator and M4A runtime remain MKE-free.

M1 freezes the visual contract; M2 adds identity/session/RLS; M3A adds deterministic planning; M3B adds the local synthetic advisor-to-family backend proof; M4A adds a local deterministic durable worker and authorized SSE replay proof. M4B has an approved read-only MKE Evidence boundary design and implementation plan, but implementation has not started. The fixture-only `/demo` remains disconnected.
