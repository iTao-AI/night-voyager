...
- DRA integration reviewers: inspect the [approved governed mixed-evidence design](superpowers/specs/2026-07-15-dra-governed-mixed-evidence-closure-design.md) and [implementation plan](superpowers/plans/2026-07-15-dra-governed-mixed-evidence-closure.md). The design and plan are approved, but implementation has not started. DRA remains an optional research provider; imported results begin as `UNTRUSTED_CANDIDATE`, human verification and approve-time promotion share one atomic authority gate, and the existing v0.1.0 synthetic `/demo` remains unchanged.
...
The governed mixed-evidence DRA closure is approved but not implemented.
