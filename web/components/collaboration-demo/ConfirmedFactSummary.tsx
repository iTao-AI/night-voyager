...
export function ConfirmedFactSummary({ fact, caseRevision }: { fact: ConfirmedFactProjection; caseRevision: number }) {
  ...
  return (
    <section className="collaboration-panel confirmed-fact" data-confirmed-record="true" data-fact-version={fact.fact_version} data-case-revision={caseRevision} aria-labelledby="confirmed-fact-title">
      ...
      <dl className="collaboration-facts">
        <div><dt>{copy("factVersionLabel")}</dt><dd>{fact.fact_version}</dd></div>
        <div><dt>{copy("caseRevisionLabel")}</dt><dd>{caseRevision}</dd></div>
