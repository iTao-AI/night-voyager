...
- **Portfolio 与 dependency boundary：** v0.1.5 包含 PR #78 合并的 frontend dependency maintenance。当前 development 已使用 Next.js 与 `eslint-config-next` `16.3.0`；Next.js resolves optional/transitive `sharp 0.35.3`，位于 `GHSA-f88m-g3jw-g9cj` advisory range 外。仓库没有 `direct sharp dependency or override`。不可变的 v0.1.5 release 不是 audit-zero claim；这段 current-development wording 不改写其 historical release evidence。Dependabot #7 hosted alert status is evaluated after merge；本地变更不作 hosted alert claim。Fresh runtime/full audit evidence 保留 approved `postcss@8.5.18` -> `nanoid@3.3.16` advisory path，以及 dev-only `brace-expansion` / `minimatch` -> ESLint chain。Recovery triggers 仍仅为 public deployment、untrusted image path、compatible upstream support for sharp >=0.35 或 advisory change。
