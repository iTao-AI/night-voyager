...
The Python runtime graph for the optional `mke` extra resolves `cryptography==50.0.0` transitively through `mcp` and `PyJWT[crypto]`. This is the first patched version for Dependabot alert #14 (`GHSA-g6cj-pr64-35w5` / `CVE-2026-69247`); the alert remains open until post-merge GitHub readback, and this local change makes no hosted alert claim. A source and test scan found no Night Voyager use of cryptography's PKCS#7 decryption or finite-field Diffie-Hellman APIs, so this upgrade requires no product compatibility shim.
