...
- **PostgreSQL and forced RLS:** tenant-scoped runtime roles read and mutate through narrow authority paths backed by the current development graph `0001 -> 0002 -> 0003 -> 0004 -> 0005 -> 0006 -> 0007 -> 0008 -> 0009 -> 0010 -> 0011 -> 0012 -> 0013 -> 0014`; the released v0.1.4 identity remains fixed at migration `0013`.
...
- **Governed timeline execution:** current development PR A adds a provider-free `/demo/plan` vertical, structured family attestations, assigned-advisor verification, immutable receipts, PostgreSQL-owned risk/date authority, and a reassessment stop. It creates no new `AgentTask`, provider call, successor business row, release, or deployment.
...
the [timeline execution runbook](docs/operations/timeline-execution.md). This PR A
...
- Governed timeline-execution PR A is current local development only. PR B recovery/reassessment presentation, PR C presentation/DX/release closure, publication, and release are not claimed.
